...
export const columns = (openModal, setId, handleUpdate) => [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center justify-start gap-2">{name}</div>
      );
    },
  },
  {
  ...
  },
  {
    ...
    header: ({ column }) => {
      return <Button variant="ghost">SUB CATEGORY</Button>;
    },
    cell: ({ row }) => {
      const count = row.getValue("subcategory_count");
      return (
        <Badge>
          {count} <span className="ml-1">Sub category</span>
        </Badge>
      );
    },
  },
